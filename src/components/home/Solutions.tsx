...
export const Solutions = () => (
  <section className="py-24 md:py-32 bg-bone">
    <div className="container-tight">
          ...
          <Reveal key={it.t} delay={i * 60}>
            <article className="group h-full bg-bone p-8 md:p-10 transition-colors duration-500 hover:bg-card">
              <div className="flex items-start justify-between mb-10">
                <div className="h-12 w-12 rounded-xl bg-background text-foreground flex items-center justify-center transition-transform duration-500 group-hover:rotate-3">
      ...
      <Reveal>
        <div className="mt-20 md:mt-28 rounded-3xl bg-background text-foreground overflow-hidden">
          <div className="grid lg:grid-cols-[1.1fr_1fr] gap-px bg-muted/50">
            <div className="bg-background p-10 md:p-14">
              <p className="eyebrow text-teal mb-5">Fortaleza · Consultoría postventa</p>
              <h3 className="heading-display text-3xl md:text-5xl text-foreground text-balance leading-[1.05]">
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                {["Cost per Repair", "First Time Fix %", "TAT E2E", "Technical Returns", "% Service Cost vs Sales"].map((k) => (
                  <span key={k} className="text-xs md:text-sm font-mono text-foreground/80 border border-border rounded-full px-3 py-1.5">
                    {k}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-background p-10 md:p-14 grid sm:grid-cols-2 gap-px bg-muted/50 sm:gap-0">
              {consultingPillars.map((p, i) => (
                <div
                  key={p.t}
                  className={`bg-background p-2 sm:p-6 ${i < 2 ? "sm:border-b" : ""} ${i % 2 === 0 ? "sm:border-r" : ""} sm:border-border`}
                >
                  ...
                  <h4 className="font-display text-lg text-foreground leading-snug mb-2">{p.t}</h4>
          ...
          <div className="border-t border-border px-10 md:px-14 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <p className="text-sm text-foreground/60 italic font-display">
