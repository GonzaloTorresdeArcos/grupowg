...
export const ServiceOSBlock = () => {
  const { t } = useTranslation("home-serviceos");
  const before = t("before", { returnObjects: true }) as string[];
  const after = t("after", { returnObjects: true }) as string[];
  return (
      ...
      <div className="container-tight relative">
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          <Reveal className="lg:col-span-5">
            ...
            <p className="mt-8 text-lg text-muted-foreground leading-relaxed">{t("lead")}</p>

            <div className="mt-10 space-y-6">
              <div>
                <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-teal mb-2">
                  {t("whatLabel")}
                </p>
                <p className="text-foreground text-base leading-relaxed">{t("whatText")}</p>
              </div>
              <div>
                <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-teal mb-2">
                  {t("doesLabel")}
                </p>
                <p className="text-foreground text-base leading-relaxed">{t("doesText")}</p>
              </div>
            </div>
          </Reveal>
