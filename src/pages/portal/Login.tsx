...
const PortalLogin = () => {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    toast.success(t("login.toasts.signupOk"), { description: t("login.toasts.signupOkDesc") });
    // Intento de login automático (si el email auto-confirm está activo)
    const { error: signInErr } = await supabase.auth.signInWithPassword({ email, password });
    if (!signInErr) navigate(from, { replace: true });
  };
                ...
                <form onSubmit={handleSignUp} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input id="signup-name" name="name" type="text" required placeholder={t("login.fields.namePlaceholder")} className="pl-9" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{t("login.fields.applicationHelp")}</p>
                  </div>
                  <div className="space-y-2">
