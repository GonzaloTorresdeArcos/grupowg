...
BEGIN
  ...
  -- SLA-E1.1 (punto 1): el literal contractual puede expresarse en HORAS laborables.
  -- Management goberno en E0.1 §24 la normalizacion 8h -> T+1 WD y 32h -> T+4 WD; la
  -- granularidad DATE es suficiente. Se conserva como informacion, NO como blocker.
  ...
  IF v_rv.calendario_requerido THEN
    ...
    ELSE
      ...
      FOR v_t IN SELECT x FROM jsonb_array_elements_text(v_terrs) x WHERE x <> 'UNRESOLVED' LOOP
        ...
        END IF;
      END LOOP;
      -- SLA-E1.1 (punto 2): el territorio no resuelto NO invalida el KPI. Las OTs
      -- afectadas se devuelven como NOT_EVALUABLE con reason=territorio_no_resuelto
      -- en la capa por OT y se informan aqui como metrica, no como blocker.
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'unidad_horas_gobernada_por', CASE WHEN v_horas
      THEN 'Decision Management E0.1 §24 (CERRADA): 8h laborables = T+1 working day; 32h laborables = T+4 working days. Granularidad DATE suficiente.'
      ELSE NULL END,
    ...
    'territorio_no_resuelto_tratamiento', 'OT-level NOT_EVALUABLE (reason=territorio_no_resuelto); no invalida el KPI (SLA-E1.1 punto 2)',
    'calendario', v_cobs);
END $function$;