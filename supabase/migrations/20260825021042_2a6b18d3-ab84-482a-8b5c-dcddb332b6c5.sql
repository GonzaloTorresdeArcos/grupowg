-- B1 (ajuste) · back_rows solo sobre OTs "vivas" en la ventana de 12 meses
-- + desempate determinista en top_tecs.
...
BEGIN
  ...
  tecnicos AS (
    ...
    ORDER BY t.n30 DESC, t.abiertas DESC, t.tecnico LIMIT 100
  ),
  tec_etapas AS (
  ...
  clientes AS (
    ...
    ORDER BY c.n30 DESC, c.abiertas DESC, c.cliente LIMIT 30
  ),
  prod AS (
  ...
  -- Solo OTs que pudieron estar vivas en algún fin de mes de la ventana.
  vivas AS (
    SELECT id, COALESCE(NULLIF(delegacion,''),'Red SAT externa') AS dele, tecnico,
  ...
  SELECT jsonb_build_object(
    ...
    'buckets', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM buckets ORDER BY bucket) e), '[]'::jsonb),
    'etapas', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM etapas ORDER BY n DESC, estado) e), '[]'::jsonb),
    'delegaciones', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM deleg_final ORDER BY n30 DESC, abiertas DESC, delegacion) e), '[]'::jsonb),
    'tecnicos', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM tecnicos ORDER BY n30 DESC, abiertas DESC, tecnico) e), '[]'::jsonb),
    'tec_etapas', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM tec_etapas ORDER BY tecnico, estado) e), '[]'::jsonb),
    'clientes', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM clientes ORDER BY n30 DESC, abiertas DESC, cliente) e), '[]'::jsonb),
    'producto', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM prod ORDER BY dim, n30 DESC, valor) e), '[]'::jsonb),
    ...
    'prov_30', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM prov_30 ORDER BY n DESC, provincia) e), '[]'::jsonb),
    'sat_30', COALESCE((SELECT jsonb_agg(row_to_json(e)) FROM (SELECT * FROM sat_30 ORDER BY n DESC, sat) e), '[]'::jsonb)
  ) INTO v;
  RETURN v;
END;
$function$;