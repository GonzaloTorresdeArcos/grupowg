-- ETAPA 3 · B1 — ops_sla se parte en RESUMEN (agregados) + DETALLE (paginado).
-- Optimización de series: back_rows genera, por OT, solo los meses en los que
-- estuvo abierta (generate_series lateral) en vez de cruzar cada mes contra
-- toda la tabla filtrada.
...
BEGIN
  ...
  bucket_est AS (
    SELECT bucket, est, COUNT(*) AS cn FROM bucket_rows GROUP BY 1,2
  ),
  bucket_pred AS (
    SELECT DISTINCT ON (bucket) bucket, est, cn FROM bucket_est ORDER BY bucket, cn DESC, est
  ),
  buckets AS (
    SELECT b.bucket, SUM(b.cn) AS total, MAX(p.est) AS estado_pred, MAX(p.cn) AS estado_pred_n
    FROM bucket_est b JOIN bucket_pred p ON p.bucket = b.bucket
    GROUP BY b.bucket
  ),
  etapas AS (
  ...
  meses6 AS (SELECT generate_series(v_m6, v_mtop, '1 month')::date AS mes),
  meses4 AS (SELECT generate_series(v_m4, v_mtop, '1 month')::date AS mes),
  -- Una fila por (OT, mes en que seguía abierta) dentro de la ventana de 12 meses.
  back_rows AS (
  ...
  back_deleg AS (
    SELECT dele, mes, COUNT(*) AS abiertas, ROUND(AVG(edad)::numeric,1) AS edad_media
    FROM back_rows WHERE mes >= v_m6 GROUP BY 1,2
  ),
  evo_deleg AS (
    SELECT d.dele AS delegacion, m.mes, COALESCE(b.abiertas,0) AS abiertas, b.edad_media
    FROM (SELECT DISTINCT dele FROM back_deleg) d
    CROSS JOIN meses6 m
    LEFT JOIN back_deleg b ON b.dele = d.dele AND b.mes = m.mes
  ),
  top_tecs AS (
...
-- Detalle bajo demanda: OTs abiertas de un tramo de antigüedad o de una etapa.
CREATE OR REPLACE FUNCTION public.ops_sla_detalle(
  p_tipo text, p_clave text,
...
DECLARE
  ...
  v jsonb;
BEGIN
  IF p_tipo IS NULL OR p_tipo NOT IN ('bucket','etapa') THEN
    RAISE EXCEPTION 'p_tipo debe ser bucket o etapa';
  END IF;

  WITH ab AS (
    SELECT f.*, (v_asof - f.fecha_creacion)::int AS edad,
      COALESCE(NULLIF(f.estado,''),'(sin estado)') AS estado_norm
    FROM public.ops_fact_ot f
    WHERE f.es_anulado = false AND f.situacion = 'Abierto'
      AND (p_delegacion IS NULL OR f.delegacion = p_delegacion)
      AND (p_cliente IS NULL OR f.cliente_wg = p_cliente)
      AND (p_gama IS NULL OR f.gama_real = p_gama)
      AND (p_familia IS NULL OR f.familia = p_familia)
  ...
  sel AS (
    SELECT * FROM ab
    WHERE CASE
      WHEN p_tipo = 'etapa' THEN estado_norm = p_clave
      ELSE (CASE
        WHEN GREATEST(edad,0) <= 5 THEN '0-5'
        WHEN edad BETWEEN 6 AND 10 THEN '6-10'
        WHEN edad BETWEEN 11 AND 20 THEN '11-20'
  ...
  pag AS (
    SELECT num_ot, cliente_wg, familia, provincia, tecnico, sat, delegacion,
      estado_norm AS estado, fecha_creacion, edad AS dias_abierta
    FROM sel ORDER BY edad DESC, fecha_creacion ASC NULLS LAST, num_ot
  ...
  SELECT jsonb_build_object(
    ...
    'limit', v_lim,
    'offset', v_off,
    'rows', COALESCE((SELECT jsonb_agg(row_to_json(p)) FROM pag p), '[]'::jsonb)
  ) INTO v;
  RETURN v;
END;
$function$;

COMMENT ON FUNCTION public.ops_sla(date,date,text,text,text,text,text,text,text,text,text)
  IS 'DEPRECATED: usar ops_sla_resumen + ops_sla_detalle. Se mantiene por compatibilidad.';