...
-- Índices para acelerar agregaciones sobre 125k+ filas
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_fecha_cierre ON public.ops_fact_ot(fecha_cierre) WHERE es_anulado = false;
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_fecha_creacion ON public.ops_fact_ot(fecha_creacion) WHERE es_anulado = false;
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_tecnico ON public.ops_fact_ot(tecnico) WHERE es_anulado = false;
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_delegacion ON public.ops_fact_ot(delegacion) WHERE es_anulado = false;
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_sat ON public.ops_fact_ot(sat) WHERE es_anulado = false;
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_provincia ON public.ops_fact_ot(provincia) WHERE es_anulado = false;
CREATE INDEX IF NOT EXISTS idx_ops_fact_ot_situacion ON public.ops_fact_ot(situacion) WHERE es_anulado = false;

-- =========================================================
-- KPIs del período (por fecha_cierre para cerradas, fecha_creacion para creadas)
-- =========================================================
...
BEGIN
  ...
  abiertas AS (
    SELECT * FROM base WHERE situacion = 'Abierto'
  )
...
-- =========================================================
-- Evolución mensual (últimos 18 meses, por fecha_cierre)
-- =========================================================
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM base WHERE situacion='Cerrado' AND date_trunc('month', fecha_cierre)::date = m.mes) AS cerradas,
    (SELECT CASE WHEN COUNT(*)>0 THEN COUNT(*) FILTER (WHERE kpi_20d)::numeric/COUNT(*) ELSE 0 END
       FROM base WHERE situacion='Cerrado' AND date_trunc('month', fecha_cierre)::date = m.mes) AS pct_sla20,
    (SELECT CASE WHEN COUNT(*)>0 THEN COUNT(*) FILTER (WHERE es_baja)::numeric/COUNT(*) ELSE 0 END
       FROM base WHERE situacion='Cerrado' AND date_trunc('month', fecha_cierre)::date = m.mes) AS pct_bajas
  FROM meses m
  ORDER BY m.mes;
$$;

-- =========================================================
-- Scorecard técnicos con Δ mes, benchmark y grupo comparable
-- =========================================================
...
BEGIN
  ...
  periodo AS (
    SELECT * FROM base WHERE situacion='Cerrado' AND fecha_cierre BETWEEN v_from AND v_to
  ),
  prev AS (
    SELECT tecnico, COUNT(*) AS n FROM base
    WHERE situacion='Cerrado' AND fecha_cierre BETWEEN v_prev_from AND v_prev_to
    GROUP BY tecnico
  ),
  bench_map AS (
    SELECT familia, cliente_wg,
      AVG(pct_bajas) AS pb, AVG(pct_nff) AS pn
    FROM public.ops_benchmark
    GROUP BY familia, cliente_wg
  ),
  tec_agg AS (
    SELECT
      p.tecnico,
  ...
  merged AS (
    SELECT
      ...
      t.motivo_inactivo,
      ta.cerradas,
      COALESCE(pv.n, 0) AS cerradas_prev,
...
BEGIN
  ...
  evo AS (
    ...
    FROM base WHERE situacion='Cerrado' AND fecha_cierre >= (CURRENT_DATE - INTERVAL '12 months')
  ...
  canal AS (
    ...
    FROM base WHERE situacion='Cerrado' GROUP BY canal
  ),
  mix AS (
    ...
    WHERE b.situacion='Cerrado' AND b.familia IS NOT NULL
...
BEGIN
  WITH caidas AS (
    ...
    WHERE f.es_anulado=false AND f.situacion='Cerrado' AND f.tecnico IS NOT NULL
  ...
  calidad AS (
    ...
    WHERE b.es_anulado=false AND b.situacion='Cerrado' AND b.tecnico IS NOT NULL
...
-- =========================================================
-- Delegaciones (KPIs por delegación + evolución)
-- =========================================================
...
-- =========================================================
-- SLA: distribución tramos, abiertas, resúmenes
-- =========================================================
...
BEGIN
  ...
  periodo AS (
    SELECT * FROM base WHERE situacion='Cerrado'
      AND (p_from IS NULL OR fecha_cierre >= p_from)
      AND (p_to IS NULL OR fecha_cierre <= p_to)
  ),
  tramos AS (
...
-- =========================================================
CREATE OR REPLACE FUNCTION public.ops_filter_options() RETURNS jsonb
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
  SELECT jsonb_build_object(
    ...
    'gamas', COALESCE((SELECT jsonb_agg(DISTINCT gama_origen ORDER BY gama_origen) FROM public.ops_fact_ot WHERE gama_origen IS NOT NULL), '[]'::jsonb),
    ...
    'tecnicos', COALESCE((SELECT jsonb_agg(DISTINCT tecnico ORDER BY tecnico) FROM public.ops_fact_ot WHERE tecnico IS NOT NULL), '[]'::jsonb),
...
-- Grants: solo usuarios autenticados; las funciones son SECURITY INVOKER
-- así que las RLS de ops_fact_ot filtran a rol management automáticamente.
GRANT EXECUTE ON FUNCTION public.ops_kpis(date,date,text,text,text,text,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_evolucion(text,text,text,text,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_tecnicos_scorecard(date,date,text,text,text,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_tecnico_ficha(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_alertas(date,date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_delegaciones(date,date,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_sla(date,date,text,text,text,text,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_sats_ranking(date,date,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.ops_filter_options() TO authenticated;
