...
kpis AS (
  SELECT
    (SELECT count(*) FROM cerr) AS cerradas,
    (SELECT count(*) FROM ab) AS abiertas,
    (SELECT count(*) FROM ab WHERE edad > 30) AS abiertas30,
    (SELECT count(*) FROM cerr WHERE provincia IS NOT NULL) AS con_provincia,
    (SELECT count(*) FROM cerr WHERE municipio IS NOT NULL) AS con_municipio,
    (SELECT count(*) FROM cerr WHERE cp_norm ~ '^\d{5}$') AS cp_valido,
    (SELECT count(*) FROM cerr c WHERE c.cp_norm ~ '^\d{5}$' AND EXISTS (SELECT 1 FROM public.ops_cp_geo g WHERE g.cp = c.cp_norm)) AS geocodificadas,
    (SELECT count(*) FROM cerr WHERE capital = 'SI') AS capital_si,
    (SELECT count(*) FROM cerr WHERE capital = 'NO') AS capital_no,
    (SELECT count(DISTINCT provincia) FROM cerr WHERE provincia IS NOT NULL) AS provincias_servidas,
    (SELECT count(DISTINCT (provincia, municipio)) FROM cerr WHERE municipio IS NOT NULL) AS municipios_servidos,
    (SELECT count(DISTINCT cp_norm) FROM cerr WHERE cp_norm ~ '^\d{5}$') AS cps_servidos,
    (SELECT count(*) FROM salidas) AS salidas_km,
    (SELECT round(percentile_cont(0.5) WITHIN GROUP (ORDER BY km_ida)::numeric, 1) FROM salidas) AS km_mediana,
    (SELECT round(avg(km_ida)::numeric, 1) FROM salidas) AS km_media,
    (SELECT coalesce(sum(km_total), 0) FROM kmreal) AS km_reales_total,
    (SELECT count(*) FROM kmreal) AS km_reales_tecnicos
),
...
uni AS (
  SELECT provincia, municipio, cp_norm, tipo_recurso, tecnico, sat FROM cerr
  UNION ALL
  SELECT provincia, municipio, cp_norm, tipo_recurso, tecnico, sat FROM ab
),
calidad AS (
  SELECT
    count(*) AS total,
    ...
    count(*) FILTER (WHERE u.cp_norm ~ '^\d{5}$' AND NOT EXISTS (SELECT 1 FROM public.ops_cp_geo g WHERE g.cp = u.cp_norm)) AS cp_no_geocodificado,
    count(*) FILTER (WHERE u.cp_norm ~ '^\d{5}$' AND pp.pref IS NOT NULL AND substring(u.cp_norm FROM 1 FOR 2) <> pp.pref
      AND EXISTS (SELECT 1 FROM public.ops_cp_geo g WHERE g.cp = u.cp_norm)) AS cp_no_casa,
