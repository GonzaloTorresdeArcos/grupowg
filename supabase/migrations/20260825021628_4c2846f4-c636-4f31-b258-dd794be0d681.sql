...
gam_km AS (
  SELECT coalesce(c.gama_real, 'Sin clasificar') AS gama,
    round(percentile_cont(0.5) WITHIN GROUP (ORDER BY s.km_ida)::numeric, 1) AS km_mediana,
    count(*) AS salidas
  FROM salidas s JOIN cerr c ON c.tecnico = s.tecnico AND c.municipio IS NOT DISTINCT FROM s.municipio AND c.provincia IS NOT DISTINCT FROM s.provincia
  GROUP BY 1
),
gamas AS (
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public
AS $fn$
WITH lim AS (SELECT LEAST(GREATEST(coalesce(p_limit, 50), 1), 500) AS l, GREATEST(coalesce(p_offset, 0), 0) AS o),
cerr AS MATERIALIZED (
  SELECT f.tecnico, f.sat, f.tipo_recurso, f.provincia, f.municipio,
    lpad(regexp_replace(COALESCE(f.codigo_postal, ''), '\D', '', 'g'), 5, '0') AS cp_norm,
  ...
  WHERE f.es_anulado = false AND f.incidencia <> 'ANULADO AVISO'
    ...
    AND (p_gama IS NULL OR f.gama_real = p_gama)
    AND (p_familia IS NULL OR f.familia = p_familia)
    AND ((p_entidad = 'provincia' AND f.provincia = p_clave)
      OR (p_entidad = 'tecnico' AND f.tecnico = p_clave))
),
ab AS MATERIALIZED (
  SELECT f.tecnico, f.sat, f.provincia, f.municipio,
    lpad(regexp_replace(COALESCE(f.codigo_postal, ''), '\D', '', 'g'), 5, '0') AS cp_norm,
    GREATEST(0, public.ops_as_of('ot') - f.fecha_creacion) AS edad
  FROM public.ops_fact_ot f
  WHERE f.es_anulado = false AND f.incidencia <> 'ANULADO AVISO'
    AND f.situacion = 'Abierto'
    AND (p_delegacion IS NULL OR f.delegacion = p_delegacion)
...
municipios AS (
  SELECT coalesce(c.provincia, a.provincia) AS provincia, coalesce(c.municipio, a.municipio) AS municipio,
    ...
    c.sla20, c.pct_bajas, c.cps, c.recursos, c.pct_fuera_capital,
...
cps AS (
  SELECT cp_norm AS cp, count(*) AS cerradas,
    max(municipio) AS municipio,
    (SELECT g.lat FROM public.ops_cp_geo g WHERE g.cp = c.cp_norm) AS lat,
    (SELECT g.lng FROM public.ops_cp_geo g WHERE g.cp = c.cp_norm) AS lng
  FROM cerr c WHERE cp_norm ~ '^\d{5}$' GROUP BY cp_norm
  ORDER BY count(*) DESC, cp_norm LIMIT 100
)
SELECT jsonb_build_object(
  'entidad', p_entidad,
  'clave', p_clave,
  'total', (SELECT count(*) FROM municipios),
  'limit', (SELECT l FROM lim),
  'offset', (SELECT o FROM lim),
  'municipios', coalesce((
    SELECT jsonb_agg(row_to_json(m)) FROM (
      SELECT * FROM municipios ORDER BY cerradas DESC, abiertas DESC, municipio
      LIMIT (SELECT l FROM lim) OFFSET (SELECT o FROM lim)
    ) m), '[]'::jsonb),
  'cps', coalesce((SELECT jsonb_agg(row_to_json(cps)) FROM cps), '[]'::jsonb)
)
$fn$;
