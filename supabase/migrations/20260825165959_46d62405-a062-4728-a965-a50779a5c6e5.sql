CREATE OR REPLACE FUNCTION public.ops_delegacion_ficha(p_delegacion text, p_from date DEFAULT NULL::date, p_to date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  -- La fecha efectiva del dato se resuelve UNA sola vez. Antes se invocaba
  -- public.ops_as_of('ot') dentro del filtro de la CTE `evo`, que el
  -- planificador evaluaba fila a fila sobre las ~46 k filas materializadas de
  -- `base` (4,14 s de los 4,2 s totales medidos). Mismo valor, mismo payload.
  v_asof := public.ops_as_of('ot');

  WITH base AS (
    SELECT tecnico, marca, cliente_wg, provincia, gama_real, situacion,
           fecha_cierre, fecha_creacion, es_baja, kpi_20d
  ...
  abiertas_prov AS (
    ...
    HAVING COUNT(*) FILTER (WHERE (v_asof - fecha_creacion) > 30) > 0
  ...
  evo AS (
    ...
    FROM base WHERE situacion IN ('Cerrado','Baja')
      AND fecha_cierre >= (v_asof - INTERVAL '12 months')
